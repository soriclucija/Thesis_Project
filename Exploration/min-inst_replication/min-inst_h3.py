...
for pupil in pupil_cols:
    for behav in behavior_cols:
        ...
        t_lin, p_lin = stats.ttest_1samp(linear_coefs, popmean=0)
        ...
        if pupil == 'baseline_z':
            ...
            t_quad, p_quad = stats.ttest_1samp(quad_coefs, popmean=0)
            row.update({
...
# ── controlled model (window_time as covariate) ──────────────────────────────
for pupil in pupil_cols:
    for behav in behavior_cols:
        linear_coefs = []
        quad_coefs   = []
